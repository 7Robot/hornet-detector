...
out = cv2.VideoWriter(
    "sliding_output.mp4", cv2.VideoWriter_fourcc(*"mp4v"), fps, (width, height)
)
...
def get_windows(img, size=640, xstride=480, ystride=480):
    ...
    windows, coords = [], []
    for y in range(0, height - size + 1, ystride):
        for x in range(0, width - size + 1, xstride):
...
for _ in tqdm(range(total_frames), desc="Traitement de la vidéo"):
    ...
    frame_with_windows = draw_sliding_windows(
        frame.copy(), window_size, xstride, ystride
    )

    patches, positions = get_windows(frame, window_size, xstride, ystride)
    batch = [
        torch.from_numpy(patch).permute(2, 0, 1).float() / 255 for patch in patches
    ]
